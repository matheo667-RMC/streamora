  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center gap-3 px-1">
        <Link href={href} className="group flex items-center gap-1.5 text-lg font-bold text-white hover:text-purple-300 transition-colors">
          {title}
      ...
      <div className="flex gap-2.5 sm:gap-3 overflow-x-auto pb-3 scrollbar-hide">
        {items.map((it) => (
          <Link key={`${it.kind}-${it.id}`} href={`/${it.kind}/${it.id}`} className="flex-none w-[120px] sm:w-[145px] md:w-[165px] group">
            <div className="relative aspect-[2/3] rounded-md overflow-hidden bg-gray-900 ring-1 ring-white/10 transition-all duration-300 group-hover:ring-purple-500/70 group-hover:scale-[1.06] group-hover:z-10 group-hover:shadow-2xl group-hover:shadow-purple-900/40">
              {it.posterUrl ? (
              ...
              {it.badge && (
                <div className="absolute top-1.5 right-1.5">
                  <span className="rounded bg-pink-600/90 px-1.5 py-0.5 text-[9px] font-bold leading-none">{it.badge}</span>
                </div>
              )}
