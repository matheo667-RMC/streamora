...
export function HeroBanner({ items }: { items: HeroItem[] }) {
  ...
  if (!hero) {
    return (
      <section className="relative h-[70vh] sm:h-[75vh] w-full overflow-hidden">
        <div className="h-full w-full bg-gradient-to-br from-emerald-950 via-[#1a1a2e] to-green-950" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 md:p-12 lg:p-16">
          <div className="mx-auto max-w-7xl">
            ...
            <h1 className="mb-3 text-3xl font-extrabold sm:text-4xl md:text-6xl">
              Bienvenue sur <span className="bg-gradient-to-r from-emerald-400 to-green-400 bg-clip-text text-transparent">Streamora</span>
            </h1>
  ...
  return (
    <section className="relative h-[70vh] sm:h-[75vh] w-full overflow-hidden">
      ...
      {/* Dark base */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#1a1a2e]/80 via-[#1a1a2e]/60 to-emerald-950/40" />
      ...
      <div className={`absolute inset-0 flex items-center transition-opacity duration-700 ${fade ? "opacity-100" : "opacity-0"}`}>
        <div className="mx-auto max-w-7xl w-full px-6 sm:px-8 md:px-12 lg:px-16 flex items-center gap-8 md:gap-12 lg:gap-16">
          ...
          <div className="flex-1 min-w-0">
            <span className="mb-3 inline-block rounded bg-emerald-600/80 px-2.5 py-1 text-xs font-semibold uppercase tracking-wider">
              {hero.category}
            </span>
          ...
          <div className="hidden sm:block flex-shrink-0">
            <div className="relative w-40 md:w-52 lg:w-64 aspect-[2/3] rounded-xl overflow-hidden shadow-2xl shadow-emerald-900/30 ring-1 ring-white/10">
              {hero.posterUrl && (
                <Image
                  src={hero.posterUrl}
                  alt={hero.title}
          ...
          {items.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                idx === currentIndex ? "w-6 bg-emerald-500" : "w-2 bg-white/30 hover:bg-white/50"
              }`}
            />
          ))}
        </div>
