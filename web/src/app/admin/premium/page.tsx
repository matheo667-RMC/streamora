        ...
        <div className="flex gap-2 mb-6">
          {(["users", "settings"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`rounded-lg px-4 py-2 text-sm font-semibold ${tab === t ? "bg-gradient-to-r from-emerald-600 to-green-600" : "bg-white/5 hover:bg-white/10"}`}>
        ...
        {tab === "settings" && s && (
          <div className="space-y-6">
            <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-4">
              <input type="checkbox" checked={s.autoMaintenance} onChange={(e) => setS({ ...s, autoMaintenance: e.target.checked })} className="h-5 w-5 accent-emerald-600" />
              <div>
            ...
            <label className="block rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <input
                ...
                className="mt-1 w-32 rounded-lg bg-black/30 border border-white/10 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none"
              />
            </label>

            <button onClick={saveSettings} className="rounded-lg bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-3 text-sm font-semibold">
            ...
            {users.map((u) => (
              <div key={u.id} className="px-4 py-3 border-b border-white/5 text-sm space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {u.role === "admin" && <span className="rounded bg-emerald-500/20 text-emerald-300 px-2 py-0.5 text-xs">admin</span>}
                ...
                {u.badges && <div className="text-xs text-green-300">Badges: {u.badges}</div>}
