...
import { getSessionStatus, deliverOrder } from "@/lib/payments";

export const dynamic = "force-dynamic";

// Polled by the client while waiting for payment. Also acts as a safety net
// that delivers the key even if the Stripe webhook was missed.
export async function GET(req: NextRequest) {
  ...
  if (!order.delivered) {
    try {
      const status = await getSessionStatus(order.paymentId);
      await deliverOrder(order.paymentId, status === "paid" || status === "no_payment_required");
    } catch {
